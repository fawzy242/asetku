...
const ProfileMenu = () => {
  ...
  const loadProfile = async () => {
    ...
    if (r.success) { setProfile(r.data); setFormData({ fullName: r.data.fullName || '', email: r.data.email || '', phoneNumber: r.data.phoneNumber || '', username: r.data.username || '' }); }
    setLoading(false);
  };
  ...
  const handlePasswordChange = async (e) => {
    ...
    setSaving(false);
    if (r.success) { setShowPasswordForm(false); setPasswordData({ oldPassword: '', newPassword: '', confirmPassword: '' }); setErrors({}); }
  };
  ...
  const handleProfileUpdate = async (e) => {
    ...
    setSaving(true);
    const r = await profileData.updateProfile(formData);
    setSaving(false);
    if (r.success) { setEditMode(false); await loadProfile(); }
  };

  const handleCancelEdit = () => { setEditMode(false); setFormData({ fullName: profile?.fullName || '', email: profile?.email || '', phoneNumber: profile?.phoneNumber || '', username: profile?.username || '' }); setErrors({}); };
  const handleCancelPassword = () => { setShowPasswordForm(false); setPasswordData({ oldPassword: '', newPassword: '', confirmPassword: '' }); setErrors({}); };
