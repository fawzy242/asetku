...
const CRUD_OPTIONS = { idField: 'employeeId', transformFormData: transformEmployeeFormData };
...
const EmployeesMenu = () => {
  ...
  const { showModal, editingRecord: editingEmployee, isSubmitting, formData, setFormData, handleCreate, handleEdit, handleClose, handleSubmit: crudHandleSubmit } = useCrudForm(INITIAL_FORM_DATA, employeesData, CRUD_OPTIONS);

  const departmentOptions = useMemo(() => {
    const base = [{ value: "", label: "All Departments" }];
    if (Array.isArray(departments)) departments.forEach(d => { if (d) base.push({ value: d, label: d }); });
    return base;
  }, [departments]);
  ...
  const { data: employees, totalCount, loading, page, setPage, pageSize, setPageSize, updateFilters, reload } = useGridData(['employees', activeTab, statusFilter, departmentFilter], fetchGridData);

  useEffect(() => { employeesData.fetchDepartments().then(r => { if (r.success && Array.isArray(r.data)) setDepartments(r.data.filter(Boolean)); }); }, []);

  const handleSearch = useCallback((search) => updateFilters({ search }), [updateFilters]);
  const handleDelete = useCallback(async (emp) => { const r = await employeesData.delete(emp.employeeId); if (r.success) { queryClient.invalidateQueries({ queryKey: ['reference', 'employees'] }); reload(); } }, [reload, queryClient]);
  const onSubmit = useCallback(async (e) => { e.preventDefault(); const success = await crudHandleSubmit(); if (success) { queryClient.invalidateQueries({ queryKey: ['reference', 'employees'] }); reload(); } }, [crudHandleSubmit, reload, queryClient]);
  ...
  const columns = useMemo(() => [
    ...
    { field: "employmentStatus", headerName: "Status", width: 120, renderCell: (p) => <Chip label={p?.value || '-'} size="small" sx={getStatusChipStyles(p?.value)} /> },
  ...
  return (
    <div className="employees-menu">
       <div className="page-header"><h1 className="page-title">Employee</h1> <PageHeader title="Employee Management" buttonText="Add Employee" onButtonClick={handleCreate} buttonIcon={<FiPlus />} /></div>
      ...
      <div className="employees-menu__table">
        <DataTable rows={employees} columns={columns} loading={loading} pageSize={pageSize} getRowId={(row) => row.employeeId} hideFooter={true} ariaLabel="Employees data table" />
      </div>
      ...
      <Modal isOpen={showModal} onClose={handleClose} title={editingEmployee ? "Edit Employee" : "Add Employee"} size="lg">
        <form onSubmit={onSubmit}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}><Select label="Employment Status" value={formData.employmentStatus || "Active"} onChange={e => setFormData({ ...formData, employmentStatus: e.target.value || "Active" })} options={EMPLOYMENT_OPTIONS} /></Grid>
            <Grid item xs={12} sm={6}><DatePickerInput label="Join Date" value={formData.joinDate || ""} onChange={e => setFormData({ ...formData, joinDate: e.target.value })} /></Grid>
          </Grid>
