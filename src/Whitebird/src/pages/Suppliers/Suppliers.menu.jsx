...
const INITIAL_FORM_DATA = { supplierName: "", contactPerson: "", phoneNumber: "", email: "", address: "" };
...
const SuppliersMenu = () => {
  ...
  const handleDelete = useCallback(async (sup) => { const r = await suppliersData.delete(sup.supplierId); if (r.success) { queryClient.invalidateQueries({ queryKey: ['reference', 'suppliers'] }); reload(); } }, [reload, queryClient]);
  ...
  const onSubmit = useCallback(async (e) => {
    ...
    if (r.success) { queryClient.invalidateQueries({ queryKey: ['reference', 'suppliers'] }); handleClose(); reload(); }
  ...
  const columns = useMemo(() => [
    ...
    { field: "isActive", headerName: "Status", width: 110, renderCell: (p) => <Chip label={p?.value ? 'Active' : 'Inactive'} size="small" sx={{ bgcolor: p?.value ? 'rgba(16, 185, 129, 0.1)' : 'rgba(107, 114, 128, 0.1)', color: p?.value ? '#10b981' : '#6b7280', fontWeight: 500, fontSize: '0.75rem', height: 24, borderRadius: '4px' }} /> },
    { field: "actions", headerName: "Actions", width: 100, sortable: false, renderCell: (p) => (<div className="table-actions"><IconButton onClick={() => handleEdit(p?.row)} title="Edit supplier" size="lg"><FiEdit2 size={18} /></IconButton><IconButton onClick={() => handleDelete(p?.row)} title="Delete supplier" variant="danger" size="lg"><FiTrash2 size={18} /></IconButton></div>) },
  ...
  return (
    <div className="suppliers-menu">
      ...
      <div className="suppliers-menu__table">
        <DataTable rows={suppliers} columns={columns} loading={loading} pageSize={pageSize} getRowId={(row) => row.supplierId} hideFooter={true} ariaLabel="Suppliers data table" />
      </div>
      ...
      <Modal isOpen={showModal} onClose={handleClose} title={editingSupplier ? "Edit Supplier" : "Add Supplier"} size="lg">
        <form onSubmit={onSubmit}>
          ...
          <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end', mt: 3 }}>
            ...
            <Button type="submit" variant="primary" loading={isSubmitting}>{editingSupplier ? "Update" : "Create"}</Button>
          </Box>
        </form>
      </Modal>
    </div>
